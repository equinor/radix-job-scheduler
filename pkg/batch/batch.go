...
func GetRadixBatchStatus(radixBatch *radixv1.RadixBatch, radixDeployJobComponent *radixv1.RadixDeployJobComponent) modelsv2.Batch {
...
func getRadixBatchJobStatusesFromRadixBatch(radixBatch *radixv1.RadixBatch, radixBatchJobStatuses []radixv1.RadixBatchJobStatus) []modelsv2.Job {
	...
	jobStatuses := make([]modelsv2.Job, 0, len(radixBatch.Spec.Jobs))
	for _, radixBatchJob := range radixBatch.Spec.Jobs {
		...
		radixBatchJobStatus := modelsv2.Job{
			Name:  jobName,
...
func GetRadixBatchStatuses(radixBatches []radixv1.RadixBatch, radixDeployJobComponent *radixv1.RadixDeployJobComponent) []modelsv2.Batch {
	return slice.Map(radixBatches, func(rb radixv1.RadixBatch) modelsv2.Batch {
...
func StopRadixBatch(ctx context.Context, radixClient versioned.Interface, radixBatch *radixv1.RadixBatch) error {
	...
	if !isBatchStoppable(radixBatch.Status.Condition) {
		return apierrors.NewBadRequestError(fmt.Sprintf("cannot stop completed batch %s", radixBatch.GetName()), nil)
	}
...
func StopRadixBatchJob(ctx context.Context, radixClient versioned.Interface, radixBatch *radixv1.RadixBatch, jobName string) error {
	...
	if !isBatchStoppable(radixBatch.Status.Condition) {
		return apierrors.NewBadRequestError(fmt.Sprintf("cannot stop the job %s in the completed batch %s", jobName, radixBatch.GetName()), nil)
	}
	...
	for jobIndex, radixBatchJob := range newRadixBatch.Spec.Jobs {
		...
		if jobStatus, ok := radixBatchJobsStatusMap[radixBatchJob.Name]; ok &&
			(internal.IsRadixBatchJobSucceeded(jobStatus) || internal.IsRadixBatchJobFailed(jobStatus)) {
			return apierrors.NewBadRequestError(fmt.Sprintf("cannot stop the job %s with the status %s in the batch %s", jobName, string(jobStatus.Phase), radixBatch.GetName()), nil)
		}
		newRadixBatch.Spec.Jobs[jobIndex].Stop = pointers.Ptr(true)
		return updateRadixBatch(ctx, radixClient, radixBatch.GetNamespace(), newRadixBatch)
	}
